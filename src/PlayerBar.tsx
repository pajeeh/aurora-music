...
import { Heart, ListMusic, Monitor, Pause, Play, Repeat, Repeat1, Shuffle, SkipBack, SkipForward, Volume2, VolumeX } from './icons';
...
export function PlayerBar({track,liked,like,playing,ready,canControl,previous,next,toggle,position,duration,seek,volume,setVolume,panel,setPanel,shuffle,setShuffle,repeat,setRepeat,grouped}:{
  track:Track;liked:boolean;like:()=>void;playing:boolean;ready:boolean;canControl:boolean;previous?:()=>void;next?:()=>void;toggle:()=>void;position:number;duration:number;seek:(value:number)=>void;volume:number;setVolume:(value:number)=>void;panel:'queue'|'devices'|'lyrics';setPanel:(value:'queue'|'devices')=>void;shuffle:boolean;setShuffle:(value:boolean)=>void;repeat:RepeatMode;setRepeat:(value:RepeatMode)=>void;grouped:boolean;
}) {
  ...
  return <footer className="playerbar compact-player">
    ...
    <div className="volume"><button aria-label="Mostrar fila" aria-pressed={panel==='queue'} onClick={()=>setPanel('queue')}><ListMusic/></button><button aria-label="Mostrar dispositivos" aria-pressed={panel==='devices'} onClick={()=>setPanel('devices')}><Monitor/></button><button className="mute-toggle" aria-label={volume===0?'Restaurar volume':'Silenciar'} aria-pressed={volume===0} onClick={()=>{if(volume>0){setLastVolume(volume);setVolume(0);}else setVolume(lastVolume);}}>{volume===0?<VolumeX/>:<Volume2/>}</button><input aria-label="Volume deste dispositivo" type="range" min={0} max={100} value={volume} onChange={event=>setVolume(Number(event.target.value))}/></div>
  </footer>;
}
