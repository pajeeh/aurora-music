...
function loadAPI() {
  ...
  loading = new Promise<PlayerAPI>((resolve, reject) => {
    ...
    function fail() {
      ...
      reject(
        new Error(
          'Não foi possível carregar o player. Verifique sua conexão e recarregue a página.'
        )
      );
    }
...
export function usePlayer(initialId: string, onEnded: () => void) {
        ...
        instance = new api.Player(target, {
          ...
          events: {
            onReady() {
              ...
              if (pending.current.autoplay) {
                instance.loadVideoById(pending.current.id);
              } else {
                instance.cueVideoById(pending.current.id);
              }
              timer = setInterval(() => {
                if (instance && active) {
                  setPosition(instance.getCurrentTime() || 0);
                  setDuration(instance.getDuration() || 0);
                  const isPlaying = instance.getPlayerState() === 1;
            ...
            onError(event) {
              ...
              setError(
                [100, 101, 150].includes(event.data)
                  ? 'Este vídeo não está disponível no player incorporado. Escolha outra faixa ou abra no YouTube.'
                  : `O player não conseguiu reproduzir este vídeo (código ${event.data}). Tente outra faixa.`
              );
            },
            onAutoplayBlocked() {
              if (active) {
                ...
                playingRef.current = false;
                setError(
                  'O navegador bloqueou a reprodução automática. Clique em Reproduzir ou no player do YouTube.'
                );
              }
            },
          },
  ...
  function load(id: string) {
    pending.current = { id, autoplay: true };
    setError('');
    setErrorCode(null);
    setPosition(0);
    setDuration(0);
    setPlaying(false);
    playingRef.current = false;
    if (readyRef.current) player.current?.loadVideoById(id);
  }
  ...
  function setPlayback(id: string, position: number, shouldPlay: boolean) {
    ...
    if (shouldPlay) player.current?.playVideo();
  ...
  function setVolume(value: number) {
    ...
    if (readyRef.current) player.current?.setVolume(value);
  }
